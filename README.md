...
[![Tests](https://github.com/boblangley/mcp-ssh/actions/workflows/test.yml/badge.svg)](https://github.com/boblangley/mcp-ssh/actions/workflows/test.yml)
...
1. Download the latest `mcp-ssh-*.dxt` file from the [GitHub releases page](https://github.com/boblangley/mcp-ssh/releases)
...
This fork publishes the npm package to GitHub Packages. Configure npm for the `@boblangley` scope before using `npx` or `npm install`:
...
#### Global Installation
```bash
npm install -g @boblangley/mcp-ssh
```

#### Local Development
```bash
git clone https://github.com/boblangley/mcp-ssh.git
cd mcp-ssh
npm install
npm start
```
